# -*- coding: utf-8 -*-
"""
Created on Thu Apr 11 14:45:41 2024
...
yijk = LpVariable.dicts('y', ((i, j, k) for i in nodes for j in nodes for k in range(ML)), 0, 1, LpBinary)
u = LpVariable.dicts('u', ((i) for i in nodes),0, 1000, LpInteger)
...
# code for assigning the same vehicle to the route
for i in nodes:
    for j in nodes:
        for k in range(ML):
            if i != j:
                problem += yijk[i, j, k] + yijk[j, i, k] == xij[i, j, k]

for i in nodes[1:]:
    for j in nodes[1:]:
        for k in range(ML):
            if i != j:
                problem += yijk[i, j, k] + yijk[j, i, k] <= 1

for k in range(ML):
    for j in nodes[1:]:
        problem += lpSum(yijk['A', j, k] for k in range(ML)) == 0
